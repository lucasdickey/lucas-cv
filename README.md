...
Phone layouts use `public/real-books/mobile.js` for shelf-filtered title/author search, paginated cover rows, and a native book-details dialog. The shared detail view returns to the desktop sidebar when the dialog closes or the viewport widens. Vertical touch gestures scroll the page; horizontal gestures turn the overview or pan a focused shelf. Camera rotation and panning use the same reduced-motion-aware damping as zoom. Check portrait (320px and 390px), short landscape, and desktop sizes when changing these controls.
